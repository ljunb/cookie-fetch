...
class CFetch {
  ...
  get = (url, options = {}) => {
    ...
    if (fn) {
      return opt['timeout'] ? convert.toBeTimeout(fn(url, opt), opt['timeout']) : fn(url, opt);
    } else {
  ...
  post  = (url, options = {}) => {
    const opt = {...defaultPostOptions, ...options};
    return opt['timeout'] ? convert.toBeTimeout(fetch(url, opt), opt['timeout']) : fetch(url, opt);
  };
